...
import br.com.kingo.reflection.NMSUtils;
...
public class FastBlockAPI {
	...
	private static Method player_connection_method_send_packet = NMSUtils.getMethod(nms_player_connection_class,
	...
	private static Method world_method_get_chunk = NMSUtils.getMethod(nms_world_class, "getChunkAt", int.class,
			int.class);
	private static Method world_method_s = NMSUtils.getMethod(nms_world_class, "s", nms_block_position_class);
	private static Method world_method_set_tile_entity = NMSUtils.getMethod(nms_world_class, "setTileEntity",
			nms_block_position_class, nms_tile_entity_class);
	private static Method block_method_get_combined = NMSUtils.getMethod(nms_block_class, "getByCombinedId", int.class);
	private static Method block_method_to_legacy_data = NMSUtils.getMethod(nms_block_class, "toLegacyData",
			nms_iblock_data_class);
	private static Method block_method_on_place = NMSUtils.getMethod(nms_block_class, "onPlace", nms_world_class,
			nms_block_position_class, nms_iblock_data_class);
	private static Method iblock_method_get_block = NMSUtils.getMethod(nms_iblock_data_class, "getBlock");
	private static Method tile_entity_method_invalidate_block_cache = NMSUtils.getMethod(nms_tile_entity_class,
			"invalidateBlockCache");
	private static Method itile_entity_method_a = NMSUtils.getMethod(nms_itile_entity_class, "a", nms_world_class,
			int.class);
	private static Method chunk_method_get_height_map = NMSUtils.getMethod(nms_chunk_class, "r");
	private static Method chunk_method_get_block_data = NMSUtils.getMethod(nms_chunk_class, "getBlockData",
	...
	private static Method chunk_method_a = NMSUtils.getMethod(nms_chunk_class, "a", nms_block_position_class,
			nms_enum_tile_entity_state);
	private static Method world_method_provider_m = NMSUtils.getMethod(nms_world_provider_class, "m");
	private static Method chunk_section_method_set_type = NMSUtils.getMethod(nms_chunk_section_class, "setType",
			int.class, int.class, int.class, nms_iblock_data_class);
	private static Method chunk_section_method_get_type = NMSUtils.getMethod(nms_chunk_section_class, "getType",
			int.class, int.class, int.class);
	private static Method block_method_remove = NMSUtils.getMethod(nms_block_class, "remove", nms_world_class,
			nms_block_position_class, nms_iblock_data_class);
	private static Method block_method_get_id = NMSUtils.getMethod(nms_block_class, "getId", nms_block_class);

	private static Constructor<?> con_block_position = NMSUtils.getConstructor(nms_block_position_class, int.class,
			int.class, int.class);
	private static Constructor<?> con_chunk_section = NMSUtils.getConstructor(nms_chunk_section_class, int.class,
			boolean.class);
	private static Constructor<?> con_packet_play_out_map_chunk = NMSUtils
			.getConstructor(nms_packet_play_out_map_chunk_class, nms_chunk_class, int.class);
	...
	private static Field player_field_player_connection = NMSUtils.getField(nms_entity_player_class,
			"playerConnection");
	...
	public static void setBlockFast(World world, int x, int y, int z, int blockId, byte data) {
		try {
			...
			if (chunk_section == empty_chunksection) {
				if (nms_block == block_air) {
					return;
				}
				Object world_provider = world_field_block_provider.get(nms_world);
				chunk_section = con_chunk_section.newInstance(j >> 4 << 4,
						world_method_provider_m.invoke(world_provider));
			...
			if (!(boolean) world_field_client_side.get(nms_world) && nms_block1 != nms_block
					&& !(boolean) world_field_capture_block_states.get(nms_world)
					|| nms_block__tile_entity_class.isAssignableFrom(nms_block.getClass())) {
			...
			if (nms_itile_entity_class.isAssignableFrom(nms_block.getClass())) {
				Object tile_entity = chunk_method_a.invoke(nms_chunk, nms_bp, check);
				if (tile_entity == null) {
					itile_entity_method_a.invoke(nms_block, nms_world,
							block_method_to_legacy_data.invoke(nms_block, nms_ibd));
	...
	public static FastBlockData getFastDataAt(Location loc) {
		try {
			...
			return new FastBlockData().a(loc, nms_world, nms_block, nms_ibd, m, b);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return null;
	...
	public static class FastBlockData {

		private Location location;
		private Material material;
		private byte data;
		private Object world, block, block_data;
		
		public FastBlockData a(Location location, Object world, Object block, Object block_data, Material material,
				byte data) {
